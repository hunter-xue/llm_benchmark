...
| F-26 | `esc` 返回上一步，`q` / `ctrl+c` 退出，各界面快捷键提示保持一致 | ✅ 已实现 |
...
| F-86 | 请求进行中显示"Fetching response..."占位文本；请求完成后自动填充内容 | ✅ 已实现 |
...
Provider A / B 各自独立配置：Name、URL、API Key、Model；其余参数与单 Provider 模式相同，两侧共享。
